...
namespace Unity
{
    ...
    struct il2cppArray : il2cppObject
    {
        ...
        void Insert(T m_tValue, uintptr_t m_uIndex)
        {
            if (m_uIndex >= m_uMaxLength)
                return;
            ...
            {
                for (uintptr_t u = m_uMaxLength; m_uIndex < u; --u)
                    operator[](u) = operator[](u - 1);
            }

            operator[](m_uIndex) = m_tValue;
        }

        void Push(T m_tValue)
        {
            if (m_uLength >= m_uMaxLength)
                return;

            operator[](m_uLength) = m_tValue;
            ++m_uLength;
        }
