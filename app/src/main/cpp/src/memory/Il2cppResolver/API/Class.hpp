        ...
        namespace Utils
        {
            ...
            void SetStaticField(Unity::il2cppClass* m_pClass, const char* m_pMemberName, void* m_pValue) {
                ...
                if (m_pField)
                    ...
                    Resolver_LOGE("Static field %s not found",m_pMemberName);
            }
            ...
            void SetStaticField(const char* m_pClassName, const char* m_pMemberName, void* m_pValue) {
                ...
                if (m_pClass)
                    ...
                    Resolver_LOGE("Class %s not found.",m_pClassName);
            ...
            void SetFieldValue(void* instance, const std::string& className, const std::string& fieldName, const T& value) {
                if(!instance) {
                    Resolver_LOGE("Null instance, field %s  not found in class %s, ", fieldName.c_str(), className.c_str());
                    return;
                }
                int fieldOffset = GetFieldOffset(className.c_str(), fieldName.c_str());
                if (fieldOffset < 0) {
                    Resolver_LOGE("Field %s  not found in class %s",fieldName.c_str(),className.c_str());
                    return;
                }
            ...
            void SetStaticFieldValue(const std::string& className, const std::string& fieldName, const T& value) {
                SetStaticField(className.c_str(), fieldName.c_str(), (void*)&value);
            }
            ...
            T GetStaticFieldValue(const std::string& className, const std::string& fieldName) {
                void* value = GetStaticField(className.c_str(), fieldName.c_str());
                if (!value) {
                    Resolver_LOGE("Field %s not found in class %s",fieldName,className);
                    return T();
                }
            ...
            template<typename T>
            T GetFieldValue(void* instance, const std::string& className, const std::string& fieldName) {
                if(!instance) {
                    Resolver_LOGE("Null instance, field %s  not found in class %s, ", fieldName.c_str(), className.c_str());
                    return T();
                }
            ...
            Ret InvokeMethod(void* instance, const std::string& className, const std::string& methodName, Args... args) {
                if(!instance) {
                    Resolver_LOGE("Null instance, invoke method %s not found in class %s, ", methodName.c_str(), className.c_str());
                    return Ret();
                }
                void* methodPointer  = GetMethodPointer(className.c_str(), methodName.c_str(),sizeof...(args));
                if (!methodPointer) {
                    Resolver_LOGE("Invoke method %s  not found in class %s",methodName.c_str(),className.c_str());
                    return Ret();
                }
            ...
            Ret InvokeStaticMethod(const std::string& className, const std::string& methodName, Args... args) {
                void* methodPointer = GetMethodPointer(className.c_str(), methodName.c_str(), sizeof...(args));
                if (!methodPointer) {
                    Resolver_LOGE("Method %s not found in class %s",methodName.c_str(),className.c_str());
                    return Ret();
                }
            ...
            template<typename T>
            T GetPropertyValue(void* instance, const std::string& className, const std::string& propertyName) {
                if (!instance) {
                    std::cerr << "Failed to get instance of class " << className << ".\n";
                    return T();
                }
                void* get_propertyPointer = GetMethodPointer(className.c_str(), ("get_" + propertyName).c_str());
                if (!get_propertyPointer) {
                    Resolver_LOGE("Get property %s not found in class %s",propertyName.c_str(),className.c_str());
                    return T();
                }
                return reinterpret_cast<T(*)(void*)>(get_propertyPointer)(instance);
            }

            template<typename T>
            void SetPropertyValue(void* instance, const std::string& className, const std::string& propertyName, const T& value) {
                if (!instance) {
                    Resolver_LOGE("Failed to get instance of class ", className.c_str());
                    return;
                }
                void* set_propertyPointer = GetMethodPointer(className.c_str(), ("set_" + propertyName).c_str());

                if (!set_propertyPointer) {
                    Resolver_LOGE("Set property %s not found in class %s",propertyName.c_str(),className.c_str());
                    return;
                }
                reinterpret_cast<void(*)(void*, T)>(set_propertyPointer)(instance, value);
            }

            template<typename T>
            T GetStaticPropertyValue(const std::string& className, const std::string& propertyName) {
                void* get_propertyPointer = GetMethodPointer(className.c_str(), ("get_" + propertyName).c_str());
                if (!get_propertyPointer) {
                    Resolver_LOGE("Get property %s not found in class %s",propertyName.c_str(),className.c_str());
                    return T();
                }
                return reinterpret_cast<T(*)(void*)>(get_propertyPointer)(NULL);
            }

            template<typename T>
            void SetStaticPropertyValue(const std::string& className, const std::string& propertyName, const T& value) {
                ...
                reinterpret_cast<void(*)(void*, T)>(set_propertyPointer)(NULL, value);
            }
