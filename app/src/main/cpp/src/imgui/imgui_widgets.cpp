...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    bool hovered, held;
    bool pressed = ButtonBehavior(total_bb, id, nullptr, &held);
    if (pressed)
    {
        *v = !(*v);
        MarkItemEdited(id);
    ...
    RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((*v || (held && hovered)) ? IM_COL32(50, 131, 193,it_anim->second.active_rect_alpha) : hovered ? IM_COL32(55, 59, 68,255) : IM_COL32(55, 59, 68,255)), true, style.FrameRounding);
    // RenderFrame(check_bb.Min, check_bb.Max, GetColorU32((held && hovered) ? ImGuiCol_FrameBgActive : hovered ? ImGuiCol_FrameBgHovered : ImGuiCol_FrameBg), true, style.FrameRounding);
    ImU32 check_col = IM_COL32(255, 255, 255, it_anim->second.active_rect_alpha);
...
/*bool ImGui::Checkbox(const char* label, bool* v)
{
   ...
   // const ImRect rect(pos, pos + ImVec2(size, 15));
    ...
    //RenderCheckMark(window->DrawList, check_bb.Min + ImVec2(pad, pad), check_col, square_sz - pad * 2.0f);
    ImVec2 pad(ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)), ImMax(1.0f, IM_FLOOR(square_sz / 3.6f)));
    window->DrawList->AddRectFilled(check_bb.Max - ImVec2(12, 12), check_bb.Max, ImColor(24, 26, 30, 255), 2.f);
    window->DrawList->AddRectFilled(check_bb.Max - ImVec2(12, 12), check_bb.Max, ImColor(66, 150, 249, it_anim->second.active_rect_alpha), 2.f);
    RenderCheckMark(window->DrawList, check_bb.Max - ImVec2(10, 10), ImColor(255, 255, 255, it_anim->second.active_rect_alpha), 8.f);

    window->DrawList->AddRect(check_bb.Max - ImVec2(12, 12), check_bb.Max, ImColor(255, 255, 255, (12 + it_anim->second.active_outline_anim)), 2.f);
    window->DrawList->AddText(check_bb.Min + ImVec2(0, 1), ImColor(255, 255, 255), label);
